...
def test_topics_one_sample(model, tokenizer, test_case, output_file, idx):
...
def test_lines_one_sample(model, tokenizer, test_case, output_file, idx):
    prompt = test_case["prompt"]
...
def longeval_eval_func(data_path, model, tokenizer, benchmark_dir):
    # if opt.task == "topics":
    scores = dict()

    dir_name = os.path.join(benchmark_dir, 'LongEval')
