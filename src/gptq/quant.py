...
class Quantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        tmp = torch.zeros(x.shape[0], device=dev) # [768]
        xmin = torch.minimum(x.min(1)[0], tmp)    # [768] 获取权重最小值
        xmax = torch.maximum(x.max(1)[0], tmp)    # [768] 获取权重最大值
        ...
        if self.sym:
            ...
            if torch.any(tmp):
                xmin[tmp] = -xmax[tmp]
        tmp = (xmin == 0) & (xmax == 0)  # 标记最大值和最小值都为0的地方
        xmin[tmp] = -1
        xmax[tmp] = +1

        if self.maxq < 0:  # int4量化，最大值15
          self.scale = xmax
          self.zero = xmin
        else:
          self.scale = (xmax - xmin) / self.maxq  # 每组最大值减最小值
          if self.sym:
...
try:
    import src.gptq.quant_cuda as quant_cuda
except:
...
def make_quant(module, names, name='', faster=False):
    ...
    for name1, child in module.named_children():
        make_quant(child, names, name + '.' + name1 if name != '' else name1, faster=faster)