...
from src.rlhf.policy.policy_datahelper import *
from src.rlhf.utils import *
from src.rlhf.metric import MeanMetric, PPLMetric, SumMetric, RealtimeMetric, Metrics
from accelerate import Accelerator
from src.rlhf.policy.utils import *
...
class PolicyTrainer():
    def __init__(self, opt, policy_model, ref_model, critic_model, reward_model, tokenizer, accelerator, **kwargs) -> None:
        self.opt = opt
        ...
        self.ppo_data_path: str = opt.ppo_data_path
        ...
        self.prompt_loader = DataLoader(
                                OnlyPromptDataset(self.opt, tokenizer, self.accelerator, mode='train'), 
                                batch_size=None, 
        ...
        if self.use_ppo_pretrain_loss:
            self.pretrain_loader = iter(DataLoader(
                                    self.pretrain_dataset_class()(self.opt, self.tokenizer, self.accelerator), 
                                    batch_size=None, 
                                    num_workers=self.opt.num_workers, 
                                    prefetch_factor=self.opt.num_prefetch, 
                                    pin_memory=True))
    ...
    def build_optimizer(self):
        params = self.get_parms(self.model, 'policy_model.', self.opt.min_lr, self.opt.eps)
        ...
        while len(self.replay_buffer) < self.num_rollouts:
            ...
            for i in range(bsz):
                ...
                if self.use_ppo_pretrain_loss:
                    ...
                    to_device(ppo_batch, self.opt.device)
    ...
    def evaluate(self, datatype='valid', **kwargs) -> Tuple[float, List]:
        ...
        valid_dataloader = DataLoader(
                                    OnlyPromptDataset(self.opt, self.tokenizer, self.accelerator, mode=datatype), 
                                    batch_size=None, 
                                    num_workers=self.opt.num_workers, 
                                    prefetch_factor=self.opt.num_prefetch, 
                                    pin_memory=True)
    ...
    def train(self):
        ...
        while epoch < self.max_epoch:
            ...
            self.train_loader = DataLoader(
                ExperienceDataset(self.replay_buffer, self.tokenizer, self.opt, self.accelerator), 
                batch_size=None, 
                num_workers=self.opt.num_workers, 
                prefetch_factor=self.opt.num_prefetch, 
                pin_memory=True)
            ...
            for step, batch in tqdm(enumerate(self.train_loader)):
                ...
                if self.train_state.total_steps % self.save_per_step == 0:
                    ...
                    if is_best:
                        self.train_state.best_score = eval_score
                        print(f'Greater than the best score {abs(eval_score)}.')
                    else:
