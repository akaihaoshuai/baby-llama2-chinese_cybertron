...
import json, gzip, json_lines, logging, torch, random
import os 
import langid
...
class RewardModelDialogDataset(IterableDataset):
    def __init__(self, opt, tokenizer, accelerator, mode: str = 'train', **kwargs) -> None:
        super().__init__()
        ...
        self.c_trunc: int = opt.max_seq_len
        
        # load data from opt.rw_data_path, support for suffix loading, format: opt.rw_data_path/xxx{mode}.json
        # file_names = sorted([file_name for file_name in os.listdir(opt.rw_data_path) if file_name.endswith(f'{mode}.json')])
        file_names = ['train.jsonl.gz']
        for file_name in file_names:
            rw_data_path = os.path.join(opt.rw_data_path, file_name)
            
            try:
                file_data = self._load_data(rw_data_path)
            except Exception as e:
                logging.warn(f"Load data from {rw_data_path} failed. {str(e)}")
                
            self.data.extend(file_data)
            logging.info(f'Got {len(file_data)} samples from {rw_data_path}')
    ...
    def _load_data(self, dpath: str):
        ...
        paragragph = data.split('{')
        for sample in paragragph:
            if 'chosen' in sample and 'rejected' in sample:
                chosen, reject = sample.split('"rejected":')
                context = chosen.split('Assistant:')[0][15:]+'Assistant:'
                chosen = [chosen[len(context)+16:-3]]
                reject = [reject[7+len(context):-3]]
                ...
                output.append((context, chosen[-1], reject[-1]))
    ...
    def _encode_sample(self, sample: Tuple[List[str], str, str]) -> Dict[str, Any]:
        ...
        context_vec = self.tokenizer.encode(self._build_prompt(context, dialog_sep='\n'), 
