...
from src.rlhf.reward.reward_datahelper import *
from src.rlhf.metric import MeanMetric, PPLMetric,SumMetric,RealtimeMetric,Metrics
...
class RewardTrainer():
    def __init__(self, opt, model: nn.Module, tokenizer, accelerator, eval_only=False) -> None:
        self.model = model
        ...
        if not eval_only:
            ...
            self.train_loader = self.build_dataloader(tokenizer, mode='train')
        ...
        self.max_epoch: int = opt.max_epoch
        self.save_per_step = opt.save_iters
        self.model_save_path = opt.model_path
        self.validation_metric = opt.validation_metric
        self.fp32_loss: bool = True
    ...
    def _run_fake_forward(self):
        ...
        self.model(torch.tensor([[self.tokenizer.eos_token_id]], dtype=torch.long, device=self.accelerator.device))
    ...
    def _save_checkpoint(self, is_best: bool, total_steps: int):
        ...
        torch.save(unwrapped_model.state_dict(), steps_model_path)
        # unwrapped_model.save_pretrained(
        #     steps_model_path,
        #     is_main_process=self.accelerator.is_main_process,
        #     save_function=self.accelerator.save,
    ...
    def build_dataloader(self, tokenizer, mode='train'):
        dataset = RewardModelDialogDataset(self.opt, tokenizer, self.accelerator, mode=mode)
    ...
    def build_optimizer(self):
        ...
        return optim.AdamW(params, lr=self.opt.min_lr, eps=self.opt.eps, betas=(self.opt.beta1, self.opt.beta2))
        # return DeepSpeedCPUAdam(params, lr=self.opt.min_lr, eps=self.opt.eps, betas=(self.opt.beta1, self.opt.beta2))

    def invsqrt_scheduler(self, warmup_iters):
        def _invsqrt_lr(step):
            return math.sqrt(warmup_iters) / math.sqrt(max(warmup_iters, step))
        def _warmup_lr(step):
            return max(step / warmup_iters, 0.1)
        def _invsqrt_lr_with_warmup(step):
            return max(_warmup_lr(step) if step < warmup_iters else _invsqrt_lr(step), 1e-8)
    ...
    def build_scheduler(self):        
        scheduler = optim.lr_scheduler.LambdaLR(
                                ...
                                lr_lambda=self.invsqrt_scheduler(self.opt.warmup_iters)
                                )
        return scheduler
    ...
    def _run_fake_forward(self):
        # a dummy forward for stage3 compatibility to avoid deadlock
        self.model(torch.tensor([[0]], dtype=torch.long, device=self.accelerator.device))      
    ...
    def train(self):
        ...
        while epoch < self.max_epoch:
            for batch in tqdm(self.train_loader, desc='train reward model.'):
                self.train_state.total_steps += 1
                
                start_time = time.time()
                
                self._record_batch_info(batch, mode='train')
                to_device(batch, self.opt.device)
                ...
                need_reset = False
                if self.train_state.total_steps % self.log_iters == 0:
                ...
                if self.train_state.total_steps % self.save_per_step == 0:
                    ...
                    if is_best:
                        self.train_state.best_score = eval_score
                        print(f'Achieved the best score {abs(eval_score)}')
                    else:
                        print(f'Did not beat the best score {abs(self.train_state.best_score)}.')
    ...
    def evaluate(self, datatype='valid', **kwargs) -> Tuple[float, List]:
        ...
        valid_dataloader = self.build_dataloader(self.tokenizer, mode=datatype)
        # print_rank_0(f'Start evaluation on {datatype} data.')
        self.model.eval()
        
        for step, batch in tqdm(enumerate(valid_dataloader), desc='eval reward model.'):
            # record some info
